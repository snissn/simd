...
func rowOffsetUint32Full(rowID uint32, stride, need, baseLen int) (int, bool) {
	if stride <= 0 || need < 0 {
		return 0, false
	}
	offset, ok := mulUint64(uint64(rowID), uint64(stride))
	...
	end := offset + uint64(need)
	if end < offset || end > uint64(baseLen) {
		return 0, false
	}
	return int(offset), true
}
...
func rowOffsetStrideFull(row, stride, need, baseLen int) (int, bool) {
	if row < 0 || stride <= 0 || need < 0 {
		return 0, false
	}
	offset, ok := mulUint64(uint64(row), uint64(stride))
	if !ok {
